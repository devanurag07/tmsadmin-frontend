  ...
  return (
    <div className="space-y-6 p-6">
          ...
          <CardContent>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-sm text-muted-foreground">
                ...
                {Math.max(
                  usageLimits.weekly,
                  getTotalRequests(analyticsData.weekly) + 50
                ).toLocaleString()}
              </div>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
          ...
          <CardContent>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-sm text-muted-foreground">
                ...
                {Math.max(
                  usageLimits.daily,
                  getTotalRequests(analyticsData.daily) + 10
                ).toLocaleString()}
              </div>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
      ...
      {(() => {
        ...
        const weeklyUsage = getUsagePercentage(
          getTotalRequests(analyticsData.weekly),
          usageLimits.weekly
        );
        const dailyUsage = getUsagePercentage(
          getTotalRequests(analyticsData.daily),
          usageLimits.daily
        );
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Number of requests per week</CardDescription>
              </CardHeader>
              <CardContent>
                <ChartContainer config={chartConfig}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Number of requests per month</CardDescription>
              </CardHeader>
              <CardContent>
                <ChartContainer config={chartConfig}>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Max 500 results</p>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {mirrorResults.length > 0
                  ? (
                      (getReadyResults(mirrorResults).length /
                        mirrorResults.length) *
                      100
                    ).toFixed(1)
                  : "0"}
                %
              </div>
              <p className="text-xs text-muted-foreground">Ready vs total</p>
            </CardContent>
          </Card>
        </div>
        ...
        {(() => {
          ...
          const sections = [
            { key: "today", title: "Today", results: categorized.today },
            {
              key: "yesterday",
              title: "Yesterday",
              results: categorized.yesterday,
            },
            {
              key: "lastWeek",
              title: "Last Week",
              results: categorized.lastWeek,
            },
            {
              key: "lastMonth",
              title: "Last Month",
              results: categorized.lastMonth,
            },
                        ...
                        {resultsToShow.map((result) => (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <Badge
                                variant={
                                  result.is_ready ? "default" : "secondary"
                                }
                      ...
                      {section.results.length > 6 && (
                        <div className="mt-4 text-center">
                          ...
                          >
                            {isExpanded
                              ? "Show less"
                              : `Show all ${section.results.length} results`}
                          </Button>
                        </div>
                      )}
                    </CardContent>
